...
(function () {
  ...
  // ---------- Input ----------
  // Trova l'oggetto spostabile più vicino al punto: prima i rettangoli (il
  // più piccolo, se sovrapposti), poi bersagli/estremi di portale entro un
  // raggio. Stessa logica di removeNearestAt, ma restituisce un riferimento
  // invece di cancellare.
  function findNearestMovable(p) {
    const rectCandidates = [];
    obstacles.forEach((o, i) => {
      if (p.x > o.x - o.w / 2 && p.x < o.x + o.w / 2 && p.y > o.y - o.h / 2 && p.y < o.y + o.h / 2) {
        rectCandidates.push({ kind: "obstacle", obj: o, area: o.w * o.h });
      }
    });
    windZones.forEach((z, i) => {
      if (p.x > z.x - z.w / 2 && p.x < z.x + z.w / 2 && p.y > z.y - z.h / 2 && p.y < z.y + z.h / 2) {
        rectCandidates.push({ kind: "wind", obj: z, area: z.w * z.h });
      }
    });
    gravityZones.forEach((z, i) => {
      if (p.x > z.x - z.w / 2 && p.x < z.x + z.w / 2 && p.y > z.y - z.h / 2 && p.y < z.y + z.h / 2) {
        rectCandidates.push({ kind: "gravity", obj: z, area: z.w * z.h });
      }
    });
    if (rectCandidates.length) {
      rectCandidates.sort((a, b) => a.area - b.area);
      return rectCandidates[0];
    }
    ...
    targets.forEach((t) => {
      ...
      if (d < bestDist) {
        bestDist = d;
        best = { kind: "target", obj: t };
      }
    });
    portals.forEach((pt) => {
      const d1 = Math.hypot(p.x - pt.x1, p.y - pt.y1);
      const d2 = Math.hypot(p.x - pt.x2, p.y - pt.y2);
      if (d1 < bestDist) {
        bestDist = d1;
        best = { kind: "portalA", obj: pt };
      }
      if (d2 < bestDist) {
        bestDist = d2;
        best = { kind: "portalB", obj: pt };
      }
    });
    return best;
  }

  function movableXY(found) {
    if (found.kind === "portalA") return { x: found.obj.x1, y: found.obj.y1 };
    if (found.kind === "portalB") return { x: found.obj.x2, y: found.obj.y2 };
    return { x: found.obj.x, y: found.obj.y };
  }

  function setMovableXY(found, x, y) {
    if (found.kind === "portalA") {
      found.obj.x1 = x;
      found.obj.y1 = y;
    } else if (found.kind === "portalB") {
      found.obj.x2 = x;
      found.obj.y2 = y;
    } else {
      found.obj.x = x;
  ...
  canvas.addEventListener("pointerdown", (evt) => {
    ...
    if (tool === "move") {
      const found = findNearestMovable(p);
      if (found) {
        const xy = movableXY(found);
        moveDrag = { ...found, offsetX: p.x - xy.x, offsetY: p.y - xy.y };
  ...
  canvas.addEventListener("pointermove", (evt) => {
    if (tool === "move" && moveDrag) {
      const p = pointFromEvent(evt);
      const nx = clamp(p.x - moveDrag.offsetX, 10, CANVAS_W - 10);
      const ny = clamp(p.y - moveDrag.offsetY, 10, GROUND_TOP + 30);
      setMovableXY(moveDrag, nx, ny);
      renderEditor();
      return;
    }
