...
(function () {
  ...
  function decodeLevel(code) {
    ...
    const safeGravity = (Array.isArray(payload.gravityZones) ? payload.gravityZones : []).slice(0, 6).map((z) => ({
      ...
      w: clampNum(z.w, 20, CANVAS_W, 200),
      h: clampNum(z.h, 20, CANVAS_H, 200),
      strength: clampNum(z.strength, 0, 1.2, 0.7),
    }));
  ...
  function finishDragShape(evt) {
    ...
    if (w >= 12 && h >= 12) {
      ...
      else if (tool === "wind") windZones.push({ x: cx, y: cy, w, h, fx: 0.0018, fy: 0 });
      else if (tool === "gravity") gravityZones.push({ x: cx, y: cy, w, h, strength: 0.7 });
    }
    dragStart = null;
    dragCurrent = null;
    updateStatus();
