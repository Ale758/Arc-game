...
(function () {
  ...
  const MECHANIC_HINTS_KEY = "arc-game-mechanic-hints-v1";
  ...
  // Libreria di livelli personali (creati con l'editor, salvati nel browser)
  const btnOpenMyLevels = document.getElementById("btn-open-mylevels");
  const overlayMyLevels = document.getElementById("overlay-mylevels");
  const myLevelsListMenu = document.getElementById("mylevels-list");
  const btnMyLevelsClose = document.getElementById("btn-mylevels-close");
  ...
  function buildLevel(level) {
    ...
    maybeShowTutorial();
    maybeShowMechanicHints(level);
  }

  // La prima volta che una meccanica compare davvero in un livello (in
  // qualunque modalità: normale, sfida del giorno o livello personalizzato),
  // un piccolo suggerimento testuale la spiega. Una volta visto, non
  // ricompare più.
  function maybeShowMechanicHints(level) {
    let seen = {};
    try {
      seen = JSON.parse(localStorage.getItem(MECHANIC_HINTS_KEY) || "{}");
    } catch (e) {
      /* mostriamo comunque gli aiuti, semplicemente potrebbero ripetersi in sessioni future */
    }
    const checks = [
      {
        key: "portal",
        present: level.portals && level.portals.length > 0,
        text: "🌀 Portali: entra in un cerchio ciano, esci dall'altro mantenendo la velocità.",
      },
      {
        key: "wind",
        present: level.windZones && level.windZones.length > 0,
        text: "💨 Corrente d'aria: spinge la pallina nella direzione della freccia finché resta dentro la zona.",
      },
      {
        key: "rotator",
        present: level.obstacles && level.obstacles.some((o) => o.type === "rotator"),
        text: "⚙️ Ingranaggio: muro rotante, aggiralo o usalo come deflettore.",
      },
      {
        key: "bouncer",
        present: level.obstacles && level.obstacles.some((o) => o.type === "bouncer"),
        text: "🟪 Trampolino: rimbalzo molto più forte del normale — occhio alla potenza del lancio.",
      },
      {
        key: "gravity",
        present: level.gravityZones && level.gravityZones.length > 0,
        text: "🎈 Zona a gravità leggera: la gravità si riduce (o si inverte) finché resti dentro la zona.",
      },
    ];
    let changed = false;
    checks.forEach((c) => {
      if (c.present && !seen[c.key]) {
        showToast(c.text);
        seen[c.key] = true;
        changed = true;
      }
    ...
    if (changed) {
      try {
        localStorage.setItem(MECHANIC_HINTS_KEY, JSON.stringify(seen));
      } catch (e) {
        /* nessun salvataggio disponibile: l'aiuto potrebbe ripresentarsi in futuro */
      }
    }
  }
  ...
  function finishLevel() {
    ...
    if (level.obstacles && level.obstacles.some((o) => o.type === "bouncer")) {
      maybeToastAchievement(Achievements.unlock("bouncer_master"));
    }
    if (level.gravityZones && level.gravityZones.length > 0) {
      maybeToastAchievement(Achievements.unlock("zero_gravity"));
    }
    ...
    if (mode === "normal") {
      ...
      const challengePack = LEVELS.filter((l) => l.id >= 20 && l.id <= 27);
      if (challengePack.length > 0 && challengePack.every((l) => (progress[l.id] || 0) > 0)) {
        maybeToastAchievement(Achievements.unlock("challenge_pack"));
      }
    } else if (mode === "daily") {
  ...
  // ---------- Libreria di livelli personali (giocabili dal menu) ----------
  ...
  function playSavedLevel(id) {
    if (!window.MyLevels) return;
    const level = window.MyLevels.get(id);
    if (!level) return;
    mode = "custom";
    currentLevelIndex = -1;
    activeLevelCode = window.LevelCodec ? window.LevelCodec.encode(level) : null;
    overlayMyLevels.classList.add("hidden");
    startPlaying(level);
  }

  function renderMyLevelsMenuPanel() {
    myLevelsListMenu.innerHTML = "";
    const entries = window.MyLevels ? window.MyLevels.list() : [];
    if (entries.length === 0) {
      myLevelsListMenu.innerHTML =
        '<p class="my-levels-empty">Non hai ancora salvato nessun livello. Creane uno con "✏️ Editor di livelli".</p>';
      return;
    }
    entries.forEach((entry) => {
      const item = document.createElement("div");
      item.className = "my-level-item";
      item.innerHTML = `
        <span class="my-level-name">${escapeHtml(entry.name)}</span>
        <div class="my-level-actions">
          <button class="btn-secondary" data-action="play">▶ Gioca</button>
          <button class="hud-btn" data-action="edit" title="Modifica">✏️</button>
          <button class="hud-btn" data-action="delete" title="Elimina">🗑</button>
        </div>
      `;
      item.querySelector('[data-action="play"]').addEventListener("click", () => {
        SoundEngine.playClick();
        playSavedLevel(entry.id);
      });
      item.querySelector('[data-action="edit"]').addEventListener("click", () => {
        SoundEngine.playClick();
        overlayMyLevels.classList.add("hidden");
        if (window.ArcEditor) window.ArcEditor.loadSavedLevel(entry.id);
      });
      item.querySelector('[data-action="delete"]').addEventListener("click", () => {
        if (!confirm(`Eliminare "${entry.name}"?`)) return;
        window.MyLevels.remove(entry.id);
        SoundEngine.playClick();
        renderMyLevelsMenuPanel();
      });
      myLevelsListMenu.appendChild(item);
    });
  }
  ...
  btnOpenMyLevels.addEventListener("click", () => {
    SoundEngine.playClick();
    renderMyLevelsMenuPanel();
    overlayMyLevels.classList.remove("hidden");
  });
  btnMyLevelsClose.addEventListener("click", () => {
    SoundEngine.playClick();
    overlayMyLevels.classList.add("hidden");
  });
